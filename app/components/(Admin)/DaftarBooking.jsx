...
*/
  ...
  function groupReservation(reservation) {
    const groupedData = Object.values(
      reservation.reduce((acc, row) => {
        if (!acc[row.noinvoice]) {
          acc[row.noinvoice] = { ...row, id_ruangan: [row.id_ruangan], rooms: [row.rooms] };
        } else {
          acc[row.noinvoice].id_ruangan.push(row.id_ruangan);
          acc[row.noinvoice].rooms.push(row.rooms);
        }
        return acc;
      }, {})
    )
  ...
  function editHandler(reservation) {
    console.log(reservation.rooms.map((key) => {return {id_ruangan: key.id, nomor: key.nomor}}));
  ...
  return (
    <div className="fixed left-0 top-16 bottom-10 right-0 md:left-64 pt-14 pb-6 md:pt-10 px-8 overflow-y-auto">
      ...
      <div className="overflow-x-auto bg-white dark:bg-gray-800 text-left shadow-lg rounded-lg">
        <table className="table-auto w-full">
          ...
          <tbody id="bookTableBody">
            {listRes.length > 0 ? (
              listRes.map((booking, index) => (
                <tr key={booking.noinvoice}>
                  <td className="border px-4 py-2">
                    {index + 1}
                  </td>
                  <td className="border px-4 py-2">
                    {booking.rooms.map((room, idx) => (
                      <p key={idx}>{room.nomor}</p>
                    ))}
                  </td>
                  ...
                  <td className="border px-4 py-2">
                    <button className="bg-green-600 hover:bg-green-500 text-white px-2 mx-1 py-1 rounded-md" onClick={() => startTransition(()=>checkIn(booking.noinvoice))}>
                      <FaSignInAlt />
                    </button>
                    <button className="bg-blue-500 text-white px-2 py-1 rounded-md mx-1" onClick={() => editHandler(booking)}>
                      <FaEdit />
                    </button>
                    <button className="bg-red-500 hover:bg-red-600 text-white px-2 mx-1 py-1 rounded-md" >
                      <FaTrash />
                    </button>
                  </td>
                </tr>
      ...
      {/* editform */}
      {selectedReservation && (
        <FormUpdate kamar={data.kamar} reservation={selectedReservation} getByDate={data.changeDate} isEditing={setselectedReservation} promo={data.promo}/>
      )}
